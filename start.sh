...
# Скрипт разработан специально для 4PDA от Foreman (http://pm.freize.net)
...
if [[ "$gitrepo" != *"padavan-ng"* ]]; then
   ...
   echo -e "$NONE 1) Alxdm \n 2) Mahtabctg \n 3) Hadzhioglu \n 4) Nick_Test \n -------------------------------------------------------------------------------"
       read -n1 -s
       case "$REPLY" in
       "1")  gitrepo="https://gitlab.com/dm38/padavan-ng.git"
             ICP=padavan-ng
             export ICP
             ...
             break ;;
       "2")  gitrepo="https://gitlab.com/mahtabctg/padavan-ng.git"
             ICP=padavan-ng
             export ICP
             sed -i s^gitrepo=.*^gitrepo=\"$(echo $gitrepo)\"^ $DIRP/$DIRC/script.config.sh
             sed -i s^ICP=.*^ICP=$(echo $ICP)^ $DIRP/$DIRC/script.config.sh
             ...
             break ;;
       "3")  gitrepo="https://gitlab.com/hadzhioglu/padavan-ng.git"
             ICP=padavan-ng
             export ICP
             sed -i s^gitrepo=.*^gitrepo=\"$(echo $gitrepo)\"^ $DIRP/$DIRC/script.config.sh
             sed -i s^ICP=.*^ICP=$(echo $ICP)^ $DIRP/$DIRC/script.config.sh
             ...
             break ;;
       "4")  gitrepo="https://gitlab.com/nick-padavan/padavan-ng.git"
             ICP=padavan-ng
             export ICP
             sed -i s^gitrepo=.*^gitrepo=\"$(echo $gitrepo)\"^ $DIRP/$DIRC/script.config.sh
             sed -i s^ICP=.*^ICP=$(echo $ICP)^ $DIRP/$DIRC/script.config.sh
             ...
             break ;;
       # "5")  gitrepo="https://gitlab.com/nick-padavan/padavan-ng.git"
       #       ICP=padavan-ng
       #       export ICP
       #       sed -i s^gitrepo=.*^gitrepo=\"$(echo $gitrepo)\"^ $DIRP/$DIRC/script.config.sh
       #       sed -i s^ICP=.*^ICP=$(echo $ICP)^ $DIRP/$DIRC/script.config.sh
